...
export function useApi({
  ...
  const finalUrl = resourcePath.startsWith(apiPrefix) ? resourcePath : `${apiPrefix}${resourcePath}`;
	...
	async function fetchResource(): Promise<void> {
      ...
      if (method === 'get' && pagination) {
        paginationQueryParams += `page=${pagination.page || ''}&limit=${pagination.limit || ''}`;
      }

			const response: IHttpResult = await client.makeRequest(`${apiBaseUrl}${finalUrl}?${paginationQueryParams}${client.queryParams}`, {
				method,
        ...options,
			});
